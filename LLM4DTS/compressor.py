...
class Encoder:
    ...
    def encode( self, 
        ...
        tokens_full = np.concatenate((tokens_full,np.full(rem,0,dtype=int)))
    ...
    def encode_batch(self,X,Y,parallels,final=False):
        ...
        for j in tqdm(range(iters)):
            ...
            for it in range(self.slide):
                # rank = gen_rank(probs[it],next_token=by[:,it]).cpu().numpy()
                # if not final:
                #     ranks[ind,it] = rank
                #     freqs[ind,it] = probs[it,np.arange(parallels),Y[ind,it]]
                # else:
                #     ranks[j:j+1,it] = rank
                #     freqs[j:j+1,it] = probs[it,np.arange(parallels),Y[j:j+1,it]]

                cumul[:,1:] = np.cumsum(probs[it]*10000000 + 1, axis = 1)
                # Encode with Arithmetic Encoder
                for i in range(parallels):
                    enc[i].write(cumul[i,:], Y[ind[i],it])
...
class Decoder:
    ...
    def decode_batch(self,len_series, parallels, final=False):
        ...
        for i in range(parallels):
            for j in range(self.timesteps):
                series_2d[i,j] = dec[i].read(cumul, self.vocab_size)
        ...
        for tj in tqdm(range(num_iters - self.timesteps // self.slide)):
            ...
            for it in range(self.slide):
                ...
                for i in range(parallels):
                    series_2d[i,j+self.timesteps+it] = dec[i].read(cumul[i,:], self.vocab_size)
            j = j + self.slide
    ...
    def decode_ranks_batch(self,len_series, parallels, ranks_list,final=False):
        ...
        for tj in tqdm(range(num_iters - self.timesteps // self.slide)):
            ...
            probs = self.predictor.model_predict(bx)
            for it in range(self.slide):
                series_2d[:,j+self.timesteps+it] = gen_next_token(probs[it],ranks_list[ind,it]).numpy()
