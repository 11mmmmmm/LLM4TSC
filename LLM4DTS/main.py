...
if __name__ == '__main__': 
    ...
    # parser.add_argument('--text_type', type=int,default=1,help='0:bad,1:dts')
    # parser.add_argument('--gpt_type', type=str,default='t',help='tp,rp')
    parser.add_argument('--text_file', type=str,default='../Dataset/Log1/test1')
    parser.add_argument('--win_size', type=int, default=256, help='the context window length and it cannot exceed the max seq length')
    parser.add_argument('--pred_len', type=int, default=1, help='the predicted tokens length')
    parser.add_argument('--max_seq_len', type=int, default=512)
    parser.add_argument('--batch_size', type=int, default=1)
    parser.add_argument('--encode_decode', type=int, default=1,help='0: encode, 1: decode')
    ...
    # os.makedirs("../Results2/" +args.text_file[11:]+f'/'+args.gpt_type +f'p_'+args.model_name+ f'_'+str(args.win_size)+ f'_'+str(args.pred_len)+ f'_'+str(args.batch_size),exist_ok=True)
    # compressed_file_name = "../Results2/" +args.text_file[11:]+f'/'+ args.gpt_type +f'p_'+args.model_name+ f'_'+str(args.win_size) + f'_'+str(args.pred_len)+ f'_'+str(args.batch_size)+f'/' 
    os.makedirs("../Results2/" +args.text_file[11:]+f'/'+args.model_name+ f'_'+str(args.win_size)+ f'_'+str(args.pred_len)+ f'_'+str(args.batch_size),exist_ok=True)
    compressed_file_name = "../Results2/" +args.text_file[11:]+f'/'+args.model_name+ f'_'+str(args.win_size) + f'_'+str(args.pred_len)+ f'_'+str(args.batch_size)+f'/' 
    ...
    if args.model_name == 'Llama':
        ...
        # if args.text_type == 1:
            # mypredictor = Llama2_7b_hf_predictor(args)
        # elif args.text_type == 0:
            # mypredictor = Llama_bad_predictor(args)
    elif args.model_name == 'Llama7':
        mypredictor = Llama2_7b_predictor(args)
    elif args.model_name == 'Llama13-hf':
        mypredictor = Llama2_13b_hf_predictor(args)
    elif args.model_name == 'Chatglm':
        mypredictor = Chatglm_predictor(args)
    elif args.model_name == 'DeepseekQwen':
        mypredictor = DeepseekQwen_predictor(args)
        ...
        mypredictor = XLNet_base_predictor(args)
    elif args.model_name == 'xlnet-large':
        mypredictor = XLNet_large_predictor(args)
    else:
        print("Error!")
